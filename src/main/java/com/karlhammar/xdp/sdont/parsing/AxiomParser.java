...
import com.karlhammar.xdp.sdont.model.SDEdgeFactory;
...
public class AxiomParser
{
	...
	private void parseDataProperty(OWLDataProperty dataProp)
	{
		...
		if(domain != null && range != null)
		{
//			OWLClassExpression superClass = df.getOWLDataSomeValuesFrom(dataProp.asDataPropertyExpression(),
//			        range.getRange());
			OWLClassExpression superClass = df.getOWLDataSomeValuesFrom(dataProp, range.getRange());//.asDataPropertyExpression(),
//                    range.getRange());
			parseAxiom(df.getOWLSubClassOfAxiom(domain.getDomain(), superClass));
		}
	}
